...
namespace pancheria.Services
{
    ...
    public class PanchoServices : IPanchoServices
    {
        private List<Pancho> _panchos = new() {
            new() { Id = 1, IsVegano = false, Aderezos = {"Mayonesa", "Mostaza" } },
            new() { Id = 2, IsVegano = false, Aderezos = {"Mostaza"} },
            new() { Id = 3, IsVegano = false, Aderezos = {} },
            new() { Id = 4, IsVegano = true, Aderezos = {"Mayonesa"} }
            };

        public List<PanchosDTO> GetPanchos()
        {
            return _panchos.Select(p => new PanchosDTO { Id = p.Id, Nombre = p.GetNombre(), Precio = p.Precio }).ToList();
        }
        ...
        public List<PanchoConAderezosDTO> GetPanchosPorAderezo(string aderezo)
        {
            ...
            {
                return panchosConAderezo.Select(p => new PanchoConAderezosDTO { Id = p.Id, Nombre = p.GetNombre(), Precio = p.Precio, Aderezos = p.Aderezos }).ToList();
            }
            else
            {
